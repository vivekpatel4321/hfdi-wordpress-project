...
if (! class_exists('Houzez_Activities')) {
	...
	class Houzez_Activities
	{
		...
		public static function get_activities()
		{
			...
			if ($is_admin_view_all) {
				$query = $wpdb->prepare(
					"SELECT * FROM {$table_name} ORDER BY activity_id DESC LIMIT %d, %d",
					$offset,
					$items_per_page
				);
				...
				$query = $wpdb->prepare(
					"SELECT * FROM {$table_name} WHERE user_id = %d ORDER BY activity_id DESC LIMIT %d, %d",
					$current_user_id,
					$offset,
					$items_per_page
				);
				$total_query = $wpdb->prepare(
					"SELECT COUNT(1) FROM {$table_name} WHERE user_id = %d",
					$current_user_id
				);
				$total = $wpdb->get_var($total_query);
			}
