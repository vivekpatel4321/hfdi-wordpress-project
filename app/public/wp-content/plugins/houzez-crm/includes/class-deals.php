...
if (! class_exists('Houzez_Deals')) {
	...
	class Houzez_Deals
	{
		...
		public function delete_deal()
		{
			...
			// Only administrators can delete deals
		...
		public function set_action_due_date()
		{
			...
			if (!$is_admin_view_all) {
				$deal = $wpdb->get_row($wpdb->prepare("SELECT user_id FROM {$data_table} WHERE deal_id = %d", $deal_id));
				if (!$deal || $deal->user_id != $user_id) {
					echo json_encode(array(
						'success' => false,
						'msg' => esc_html__("You don't have rights to perform this action", 'houzez-crm')
					));
					wp_die();
				}
			}
		...
		public function add_new_deal()
		{
			...
			// Check if user has permission to add deals
			$is_allowed = current_user_can('manage_options') || current_user_can('edit_others_posts');
			if (!$is_allowed) {
				echo json_encode(array(
					'success' => false,
					'msg' => esc_html__('You do not have permission to add deals', 'houzez-crm')
				));
				wp_die();
			}
		...
		public static function get_deals()
		{
			...
			if ($is_admin_view_all) {
				$sql = $wpdb->prepare(
					"SELECT * FROM {$table_name} WHERE deal_group = %s ORDER BY deal_id DESC LIMIT %d, %d",
					$deal_group,
					$offset,
					$items_per_page
				);
				$count_sql = $wpdb->prepare(
					"SELECT COUNT(1) FROM {$table_name} WHERE deal_group = %s",
					$deal_group
				);
			} else {
				$sql = $wpdb->prepare(
		...
		public static function get_total_deals_by_group($group)
		{
			...
			if ($is_admin_view_all) {
				$sql = $wpdb->prepare(
					"SELECT COUNT(*) FROM {$table_name} WHERE deal_group = %s",
					$deal_group
				);
			} else {
				$sql = $wpdb->prepare(
