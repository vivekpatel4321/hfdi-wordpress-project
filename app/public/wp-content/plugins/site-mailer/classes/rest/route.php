...
namespace SiteMailer\Classes\Rest;
...
abstract class Route {
	 ...
	 * @var string
	 */
	protected $namespace = 'site-mailer/v1';
	...
	public function respond_error_json( array $data ): WP_Error {
		if ( ! isset( $data['message'] ) || ! isset( $data['code'] ) ) {
			_doing_it_wrong(
				__FUNCTION__,
				esc_html__( 'Both `message` and `code` keys must be provided', 'site-mailer' ),
				'1.0.0'
			); // @codeCoverageIgnore
		}
		...
		return new WP_Error(
			...
			$data['message'] ?? esc_html__( 'Internal server error', 'site-mailer' ),
		);
	}
	...
	public function verify_capability( $capability = 'manage_options' ) {
		if ( ! current_user_can( $capability ) ) {
			return $this->respond_error_json([
				'message' => esc_html__( 'You do not have sufficient permissions to access this data.', 'site-mailer' ),
	...
	public function respond_wrong_method( $message = null, int $code = 404 ): WP_Error {
		if ( null === $message ) {
			$message = __( 'No route was found matching the URL and request method', 'site-mailer' );
		}
	...
	public function verify_nonce( $nonce = '', $name = '' ) {
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $nonce ) ), $name ) ) {
			return $this->respond_error_json([
				'message' => esc_html__( 'Invalid nonce', 'site-mailer' ),
				'code' => 'bad_request',
			]);
		}
	}
