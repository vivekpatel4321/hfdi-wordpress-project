...
class Houzez_Agent_Telegram_Button extends Widget_Base {
    use \HouzezThemeFunctionality\Elementor\Traits\Houzez_Preview_Query;
	use \HouzezThemeFunctionality\Elementor\Traits\Houzez_Button_Traits;

	public function get_name() {
		return 'houzez-agent-telegram-btn';
	}

	public function get_title() {
		return __( 'Agent Telegram Button', 'houzez-theme-functionality' );
	}

	public function get_icon() {
		return 'houzez-element-icon houzez-single-agent eicon-button';
	}
	...
	public function get_keywords() {
		return [ 'houzez', 'agent telegram', 'button' ];
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			'section_title',
			[
				'label' => esc_html__( 'Telegram Button', 'houzez-theme-functionality' ),
	...
	protected function render() {
        ...
        $this->single_agent_preview_query();

        $migrated = isset( $settings['__fa4_migrated']['selected_icon'] );
        $is_new = empty( $settings['icon'] ) && Icons_Manager::is_migration_allowed();

        $telegram_username = get_post_meta( get_the_ID(), 'fave_agent_telegram', true );
        ...
        <div <?php $this->print_render_attribute_string( 'wrapper' ); ?>>
            ...
            <a <?php $this->print_render_attribute_string( 'button' ); ?> target="_blank" href="<?php echo houzezStandardizeTelegramURL($telegram_username);?>">
...
Plugin::instance()->widgets_manager->register( new Houzez_Agent_Telegram_Button );