...
class Houzez_Elementor_Inquiry_Form extends Widget_Base {
    ...
    protected function houzez_form_fields_render_attributes( $i, $instance, $item ) {
        ...
        if ( isset($item['validation_message']) && ! empty( $item['validation_message'] ) ) {
            ...
            // Special message for mobile/phone fields
            if ( in_array( $item['field_type'], ['mobile', 'home_phone', 'work_phone'] ) ) {
                $input_title = 'Please enter a valid 10-digit phone number';
            } else {
        ...
        // Add phone number validation for mobile/phone fields
        if ( in_array( $item['field_type'], ['mobile', 'home_phone', 'work_phone'] ) ) {
            $this->add_render_attribute( 'input' . $i, 'pattern', '[0-9]{10}' );
            $this->add_render_attribute( 'input' . $i, 'data-rule-pattern', 'true' );
        }
    }
