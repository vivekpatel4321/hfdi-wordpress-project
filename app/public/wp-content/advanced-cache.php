...
// WP-Optimize advanced-cache.php (written by version: 4.3.1) (homeurl: https://staging-propertypoint.hfgroup.co.ke/) (abspath: /var/www/property_point/app/public/) (do not change this line, it is used for correctness checks)
...
$possible_plugin_locations = array(
	...
	'/var/www/property_point/app/public/wp-content/plugins/wp-optimize/cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('https://staging-propertypoint.hfgroup.co.ke/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', '/var/www/property_point/app/public/wp-content/plugins/wp-optimize/cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-staging-propertypoint.hfgroup.co.ke.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-staging-propertypoint.hfgroup.co.ke.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-staging-propertypoint.hfgroup.co.ke.php');
}
